...
import { FileSpreadsheet, Search, Truck, TrendingUp, TrendingDown, Calendar, ArrowRightLeft, Banknote, CreditCard, ShieldAlert } from 'lucide-react';
import { useApp } from '../App';
import { PaymentMethod, Supplier, SupplierPayment, SupplierTransaction } from '../services/types';

const SupplierReconciliationPage: React.FC = () => {
  const { suppliers, supplierPayments, supplierTransactions, setSupplierPayments, setSupplierTransactions, setSuppliers } = useApp();
  const [selectedSupplierId, setSelectedSupplierId] = useState<string>('');
  const [isPaymentModalOpen, setIsPaymentModalOpen] = useState(false);
  const [newPayment, setNewPayment] = useState({ amount: '', method: PaymentMethod.CASH, note: '' });
  ...
  const transactions = useMemo(() => {
    ...
    const payments = supplierPayments.filter(p => p.supplierId === selectedSupplierId).map(p => ({
      id: p.id,
      date: p.date,
  ...
  const handleAddPayment = (e: React.FormEvent) => {
    ...
    setSuppliers(prev => prev.map(s => 
      s.id === selectedSupplierId ? { ...s, balance: Math.max(0, s.balance - amount) } : s
    ));
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative w-full sm:w-64">
            ...
            <select value={selectedSupplierId} onChange={(e) => setSelectedSupplierId(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl focus:outline-none">
          ...
          {selectedSupplierId && (
            <button onClick={() => setIsPaymentModalOpen(true)} className="bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold shadow-lg">Выплатить долг</button>
          )}
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200">
               ...
               <h3 className="text-2xl font-black text-slate-800">{formatPrice(transactions.filter(t => t.type === 'supply').reduce((sum, t) => sum + t.amount, 0))} сум</h3>
            </div>
            <div className="bg-white p-6 rounded-3xl border border-slate-200">
               <p className="text-xs font-bold text-slate-400 uppercase mb-1">Всего выплачено</p>
               <h3 className="text-2xl font-black text-emerald-600">{formatPrice(transactions.filter(t => t.type === 'payment').reduce((sum, t) => sum + t.amount, 0))} сум</h3>
            </div>
          </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {transactions.map(t => (
                    <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right font-bold text-rose-600 text-sm">{t.type === 'supply' ? formatPrice(t.amount) : ''}</td>
                      <td className="px-6 py-4 text-right font-bold text-emerald-600 text-sm">{t.type === 'payment' ? formatPrice(t.amount) : ''}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl">
            ...
            <form onSubmit={handleAddPayment} className="space-y-4">
              <input required type="number" placeholder="Сумма оплаты (сум)" value={newPayment.amount} onChange={(e) => setNewPayment({ ...newPayment, amount: e.target.value })} className="w-full px-4 py-3 bg-slate-50 border rounded-xl focus:ring-2 focus:ring-indigo-500 focus:outline-none" />
              <select value={newPayment.method} onChange={(e) => setNewPayment({ ...newPayment, method: e.target.value as PaymentMethod })} className="w-full px-4 py-3 bg-slate-50 border rounded-xl">
              ...
              <input type="text" placeholder="Примечание" value={newPayment.note} onChange={(e) => setNewPayment({ ...newPayment, note: e.target.value })} className="w-full px-4 py-3 bg-slate-50 border rounded-xl" />
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setIsPaymentModalOpen(false)} className="flex-1 py-3 rounded-xl border">Отмена</button>
                <button type="submit" className="flex-1 py-3 rounded-xl bg-indigo-600 text-white font-bold">Выплатить</button>
              </div>
            </form>
          </div>
        </div>
